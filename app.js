/* v15.1 (diagnostic) - adds app_loaded event in index.html, keeps quiz_start */
const BUILD = '2025-10-19-10';
...
const FIXED_TAGS  = ["センサ・トランスデューサ","医用電子回路","医療情報","生体物性","電気・電子","電気的安全対策"];

const $ = (q) => document.querySelector(q);
const $$ = (q) => Array.from(document.querySelectorAll(q));

const state = { screen:'home', all:[], filtered:[], idx:0, tagFilter:'', yearFilter:'', store:loadStore(), session:null };

function loadStore(){ try{ const r=localStorage.getItem(STORE_KEY); if(r) return JSON.parse(r);}catch(e){} return {perQ:{},perTag:{},last:{screen:'home',tag:'',year:'',idx:0}};}
function saveStore(){ try{ localStorage.setItem(STORE_KEY, JSON.stringify(state.store)); }catch(e){} }
function pushLog(e){ try{ const logs = JSON.parse(localStorage.getItem(LOG_KEY)||'[]'); logs.unshift({...e,t:Date.now()}); while(logs.length>200) logs.pop(); localStorage.setItem(LOG_KEY,JSON.stringify(logs)); }catch(e){} }
function readLogs(n=50){ try{ return JSON.parse(localStorage.getItem(LOG_KEY)||'[]').slice(0,n);}catch(e){return [];} }
function resetAllLogs(){ try{ localStorage.removeItem(STORE_KEY); localStorage.removeItem(LOG_KEY);}catch(e){} location.reload(); }

function startCountdown(){ const node=$('#countdown'); const target=new Date(DATE_TARGET); function tick(){ const now=new Date(); const days=Math.max(0, Math.ceil((target-now)/(1000*60*60*24))); if(node) node.textContent=`試験日まで残り ${days} 日`; } tick(); setInterval(tick,60*1000); }
...
async function boot(){
  ...
  try{
    ...
    if(!res.ok) throw new Error('questions.json fetch error '+res.status);
    const data=await res.json();
    if(!Array.isArray(data)) throw new Error('questions.json must be array');
    state.all=data;
  }catch(e){
    console.error(e);
    const node=document.createElement('div'); node.className='card'; node.style.borderColor='rgba(239,68,68,.35)'; node.innerHTML='<strong style="color:#ef4444">questions.json の読み込みに失敗しました。</strong>'; document.querySelector('main').prepend(node);
    return;
  }
...
function initHome(){
  ...
  const updateCount=()=>{ const c=estimateCount({year:ysel.value, tag:tsel.value}); $('#homeCount').textContent=`該当 ${c} 問`; };
  ysel.addEventListener('change', updateCount); tsel.addEventListener('change', updateCount);

  const yNode=$('#homeYears'); yNode.innerHTML='';
  FIXED_YEARS.forEach(y=>{ const c=estimateCount({year:y, tag:tsel.value}); const div=document.createElement('div'); div.className='tile'; div.innerHTML=`<h3>${escapeHTML(y)}</h3><div class="muted">${c}問</div>`; div.addEventListener('click',()=>{ ysel.value=y; updateCount(); }); yNode.appendChild(div); });
  ...
  FIXED_TAGS.forEach(t=>{ const c=estimateCount({year:ysel.value, tag:t}); const div=document.createElement('div'); div.className='tile'; div.innerHTML=`<h3>${escapeHTML(t)}</h3><div class="muted">${c}問</div>`; div.addEventListener('click',()=>{ tsel.value=t; updateCount(); }); tNode.appendChild(div); });
...
function estimateCount({year='', tag=''}){
  return state.all.filter(q=>{ const tags=(q.tags||[]).map(String); const my=matchYearTag(tags,year); const mt=!tag || tags.includes(String(tag)); return my && mt; }).length;
}

function startFromHome({year='', tag=''}={}){
  try{
    const selectedYear=year||''; const selectedTag=tag||'';
    if(typeof gtag==='function'){ gtag('event','quiz_start',{ year:selectedYear, tag:selectedTag }); console.log('[GA] quiz_start sent', {year:selectedYear, tag:selectedTag}); }
    else { console.warn('[GA] gtag is not defined'); }
...
function initFilters(){ const tagSel=$('#tagFilter'), yearSel=$('#yearFilter'); FIXED_TAGS.forEach(t=>tagSel.insertAdjacentHTML('beforeend',`<option value="${escapeAttr(t)}">${escapeHTML(t)}</option>`)); FIXED_YEARS.forEach(y=>yearSel.insertAdjacentHTML('beforeend',`<option value="${escapeAttr(y)}">${escapeHTML(y)}</option>`)); }

function applyFilters(){
  const tag=($('#tagFilter')?.value ?? state.tagFilter) || ''; const year=($('#yearFilter')?.value ?? state.yearFilter) || '';
  state.filtered = state.all.filter(q=>{ const tags=(q.tags||[]).map(String); const my=matchYearTag(tags,year); const mt=!tag || tags.includes(String(tag)); return my && mt; });
  state.idx=0; state.tagFilter=tag; state.yearFilter=year;
}

function matchYearTag(tagsArr, year){
  if(!year) return true; const y=String(year);
  return tagsArr.some(s0=>{ const s=String(s0); if(y==='original'||y==='過去問') return s===y; if(s==='original'||s==='過去問') return false; if(/^\d{4}$/.test(y)){ return s===y || s.includes(y) || s.replace(/年$/,'')===y; } return s===y; });
}
...
function render(){
  ...
  const total=state.filtered.length; const qtext=$('#qtext'), choices=$('#choices'), qimg=$('#qimage'), explain=$('#explain');
  if(!total){ qtext.textContent='該当する問題がありません。トップページまたはフィルタを変更してください。'; choices.innerHTML=''; qimg.classList.add('hidden'); explain.classList.add('hidden'); $('#progress').textContent=''; $('#qmeta').textContent=''; $('#nextBtn').disabled=true; return; }
...
function renderChoices(q){
  const wrap=$('#choices'); wrap.innerHTML=''; const multi=Array.isArray(q.answerIndex); const n=(q.choices||[]).length; const order=Array.from({length:n},(_,i)=>i); shuffle(order);
  order.forEach(origIdx=>{
    const text=q.choices[origIdx]; const btn=document.createElement('button'); btn.className='choice'; btn.setAttribute('data-idx', String(origIdx)); btn.setAttribute('aria-pressed','false'); btn.innerHTML=escapeHTML(String(text));
...
function grade(){
  const q=state.filtered[state.idx]; const selectedNodes=$$('#choices .choice.selected');
  ...
  const selected=selectedNodes.map(el=>Number(el.getAttribute('data-idx'))); const result=isCorrectAnswer(selected, q.answerIndex); const correctSet=toSet(q.answerIndex);
  ...
  bumpScore(q, result.ok, selected); if(state.session){ if(result.ok) state.session.correct+=1; }
...
function openStats(){
  ...
  const weakTbody=$('#weakTable tbody'); weakTbody.innerHTML=''; const weakTags=rows.filter(r=>r.attempts>=5).sort((a,b)=> a.rate-b.rate).slice(0,10);
  ...
  const logs=readLogs(50); const logNode=$('#logList'); logNode.innerHTML=logs.map(L=>{ const dt=new Date(L.t).toLocaleString('ja-JP'); const ans=Array.isArray(L.answerIndex)?`[${L.answerIndex.join(',')}]`:String(L.answerIndex); const sel=Array.isArray(L.selected)?`[${L.selected.join(',')}]`:String(L.selected); const tagStr=(L.tags||[]).map(t=>`<span class="tag">${escapeHTML(String(t))}</span>`).join(''); return `<div style="padding:8px 0; border-bottom:1px dashed rgba(15,23,42,.1);"><div class="muted">${dt}</div><div>${L.ok?'✅ 正解':'❌ 不正解'} / 問題ID: ${escapeHTML(String(L.id))}</div><div class="muted">選択: ${sel} / 正解: ${ans}</div><div>${tagStr}</div></div>`; }).join('');
  dlg.showModal();
}

function bumpScore(q, ok, selected){
  const id=q.id ?? `idx:${state.idx}`; const pq=state.store.perQ?.[id]||{attempts:0,correct:0}; pq.attempts+=1; if(ok) pq.correct+=1; state.store.perQ=state.store.perQ||{}; state.store.perQ[id]=pq;
...
function isCorrectAnswer(userSelectedIndices, answerIndex){
  if(Array.isArray(answerIndex)){ const correct=[...answerIndex].sort((a,b)=>a-b); const user=[...new Set(userSelectedIndices)].sort((a,b)=>a-b); const partial=intersectCount(user,correct); if(correct.length!==user.length) return {ok:false, partial, total:correct.length}; const ok=correct.every((v,i)=>v===user[i]); return {ok, partial: ok?correct.length:partial, total: correct.length}; }
